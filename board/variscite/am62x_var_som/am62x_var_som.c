...
void efi_add_known_memory(void)
{
	...
	for (i = 0; i < CONFIG_NR_DRAM_BANKS; i++) {
		...
		if (gd->bd->bi_dram[i].size) {
			printf("%s ram_start=0x%llx ram_end=0x%llx \n", __func__, ram_start, ram_end);
			ram_start = (uintptr_t)map_sysmem(gd->bd->bi_dram[i].start, 0);
			if (gd->ram_size == SZ_512M) {
				ram_end = ram_start + gd->bd->bi_dram[i].size - RESERVED_MEMORY;
				printf("%s 512 ram_start=0x%llx ram_end=0x%llx \n", __func__, ram_start, ram_end);
			} else {
				ram_end = ram_start + gd->bd->bi_dram[i].size;
				printf("%s more than 512 ram_start=0x%llx ram_end=0x%llx \n", __func__, ram_start, ram_end);
			}
